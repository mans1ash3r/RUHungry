...
public class RUHungry {
    ...
    public void menu(String inputFile) {
        ...
        for (int i = 0; i < numOfCategories; i++) {
            ...
            for (int j = 0; j < numOfDishes; j++) {
                ...
                int[] ingId = new int[numofIngredients];
                for (int n = 0; n < numofIngredients; n++) {
                    ingId[n] = StdIn.readInt();
                }
                Dish dish = new Dish(categoryVar[i], dishName, ingId);
    ...
    public void addStockNode ( StockNode newNode ) {
        int ingID = newNode.getIngredient().getID();
        int index = ingID % stockVarSize;
    ...
    public void updatePriceAndProfit() {
        for (int index = 0; index < menuVar.length; index++) {
            MenuNode currentNode = menuVar[index];
            while (currentNode != null) {
                double totalCost = 0;
                double finalPrice = 0;
                int[] ingredientIDs = currentNode.getDish().getStockID();
                for (int id : ingredientIDs) {
                    StockNode ingredientNode = findStockNode(id);
                    totalCost += ingredientNode.getIngredient().getCost();
                }
                finalPrice = totalCost * 1.2;
                currentNode.getDish().setPriceOfDish(finalPrice);
                currentNode.getDish().setProfit(finalPrice - totalCost);
    ...
    public void createStockHashTable (String inputFile){
        ...
        while(!StdIn.isEmpty()){
            ...
            StdIn.readChar(); 
            String name = StdIn.readString() + StdIn.readLine(); 
            Double cost = StdIn.readDouble();
            int amount = StdIn.readInt();
            Ingredient newIngredient = new Ingredient(iD, name, amount, cost);
    ...
    public void order (String dishName, int quantity){
        boolean acceptOrder = checkDishAvailability(dishName, quantity);
        Dish dish = findDish(dishName).getDish();
        double profit = dish.getProfit()*quantity;
        if( acceptOrder==true){
            TransactionData newOrder = new TransactionData("order", dishName, quantity,profit, true );
            addTransactionNode(newOrder); 
            int[] stockIDs = dish.getStockID();
            for( int stockID: stockIDs){
                updateStock(dishName,stockID,-quantity);
            }
        }else{
            TransactionData unsucessful = new TransactionData("order", dishName, quantity, 0, false);
            addTransactionNode(unsucessful);
            MenuNode startNode = findDish(dishName);
            int index = findCategoryIndex(startNode.getDish().getCategory());
            MenuNode currentNode = startNode.getNextMenuNode() != null ? startNode.getNextMenuNode() : menuVar[index];
            boolean foundAlternative = false;
            while (!foundAlternative && currentNode != startNode) {
                if (checkDishAvailability(currentNode.getDish().getName(), quantity)) {
                    TransactionData newOrder = new TransactionData("order", currentNode.getDish().getName(), quantity, currentNode.getDish().getProfit() * quantity, true);
                    addTransactionNode(newOrder);
                    for (int i : currentNode.getDish().getStockID()) {
                        updateStock(findStockNode(i).getIngredient().getName(), i, -quantity);
                    }
                    foundAlternative = true;
                } else {
                    TransactionData newOrder = new TransactionData("order", currentNode.getDish().getName(), quantity, 0, false);
                    addTransactionNode(newOrder);
                }
                currentNode = currentNode.getNextMenuNode() != null ? currentNode.getNextMenuNode() : menuVar[index];
            }
        }
	// WRITE YOUR CODE HERE 
    }
    ...
    public void donation (String ingredientName, int quantity){
        TransactionData donationTransaction = new TransactionData("donation", ingredientName, quantity, 0, false);
        double dailyProfit = profit();
        boolean isDailyProfitGreater = dailyProfit > 50;
        StockNode ingredientStockNode = findStockNode(ingredientName);
        boolean isStockAvailable = ingredientStockNode.getIngredient().getStockLevel() > quantity;
        Ingredient ingredient = ingredientStockNode.getIngredient();
        int ingredientID = ingredient.getID();

        if (isDailyProfitGreater && isStockAvailable) {
            if (ingredientStockNode != null) {
    ...
    public void restock (String ingredientName, int quantity){
        ...
        if (ingredientStockNode != null) {
            ...
            TransactionData restockTransaction = new TransactionData("restock", ingredientName, quantity, 0, true);
            
            if (cost <= dailyProfit) {
                restockTransaction.setProfit(-cost);
                addTransactionNode(restockTransaction);
                updateStock(ingredientName, ingredientID, quantity);
            } else {
                restockTransaction.setSuccess(false);
                addTransactionNode(restockTransaction);
            }
        } else {
            TransactionData restockTransaction = new TransactionData("restock", ingredientName, quantity, 0, false);
            addTransactionNode(restockTransaction);
        }
	// WRITE YOUR CODE HERE
    }
